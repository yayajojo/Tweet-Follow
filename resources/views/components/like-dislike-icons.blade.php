<div class="flex p-6">
            <form class="mx-3" action="{{route('tweets.like',$tweet)}}" method="post">
               @csrf 
               <button type="submit">
                    <svg class="w-5 {{$tweet->likedBy(Auth::id())?'bg-green-300':'bg-gray-100'}}" viewBox="0 0 20 20">
            ...
            <form class="mx-3" action="{{route('tweets.dislike',$tweet)}}" method="post">
                @csrf
                <button type="submit">
                    <svg class="w-5 {{$tweet->dislikedBy(Auth::id())?'bg-red-300':'bg-gray-100'}}" viewBox="0 0 20 20">
